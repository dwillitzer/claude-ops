...
description: "GOD MODE - Agent #38/43 - Conclusion section specialist | Synthesizes study contributions, final takeaways, and forward-looking vision for research area"
triggers:
  - "write conclusion"
  - "final synthesis"
  - "closing remarks"
...
hooks:
  pre: |
    echo "🎯 Conclusion Writer (GOD MODE) synthesizing for: $TASK"
    npx claude-flow memory retrieve --key "phd/research-questions"
    npx claude-flow memory search --vector-mode "similar" --namespace "research/conclusions" 2>/dev/null || true
  post: |
    echo "✅ Conclusion section complete with impact vision - Vectorized"
    ...
    if (( $(echo "$DRIFT_SCORE > 0.4" | bc -l 2>/dev/null || echo 0) )); then
      ...
      npx claude-flow memory store "drift_warning" '{"agent":"conclusion-writer","score":"'$DRIFT_SCORE'","status":"flagged"}' --namespace "system/quality/drift"
    else
      echo "✅ Vector Drift Check PASSED ($DRIFT_SCORE <= 0.4)"
    fi
    npx claude-flow memory store "conclusion_output" '{"vector":true, "drift_score":"'$DRIFT_SCORE'"}' --namespace "research/conclusions"
    ...
    npx claude-flow reasoning feedback \
      ...
      --context "conclusion-writer_task_completion" 2>/dev/null || true
---
...
- **Output Key**: `research/conclusions/conclusion_v1`
- **Entropy Score**: [value]
...
npx claude-flow@alpha hooks xp-reward \
  ...
  --xp 50 \
  --reason "Completed powerful synthesis conclusion"
```
