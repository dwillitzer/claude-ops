...
type: meta-analyst
color: "#1A237E"
description: GOD MODE - Use PROACTIVELY to create comprehensive research plans using ReWOO methodology (Reasoning WithOut Observation). MUST BE USED to plan all research tasks upfront before execution. Works for ANY domain - creates complete roadmap with dependencies, resources, and quality gates.
capabilities:
  - rewoo_planning
  - task_sequencing
  - dependency_mapping
...
hooks:
  pre: |
    echo "📋 Research Planner (GOD MODE) creating comprehensive plan for: $TASK"

    # CAUSAL SIMULATION: Predict consequences of research plan BEFORE execution
    # This prevents wasted effort on plans with foreseeable failures
    echo "⚡ Running causal inference on proposed research plan..."
    bin/reasoning causal-infer \
      --action "execute research plan: $TASK" \
      --namespace "research/execution" || {
      echo "❌ FATAL: Causal inference failed - cannot validate plan safety"
      exit 1
    }
    echo "✅ Causal consequences analyzed - plan safety verified"

    npx claude-flow memory retrieve --key "research/meta/self-ask-questions"
    # GOD MODE: Use ReasoningBank for intelligent plan retrieval
    npx claude-flow reasoning ask \
      --query "research execution patterns" \
      --type "hybrid" \
      --enhance-gnn true \
      --namespace "research/execution" 2>/dev/null || true

    # CRITICAL: T0 - Seed Global Namespace for Interdisciplinary Pollinator
    echo "🌱 T0: Checking if global namespace is seeded with axioms..."
    GLOBAL_COUNT=$(npx claude-flow memory list --namespace "global" --count 2>/dev/null || echo "0")
    if [ "$GLOBAL_COUNT" -lt "10" ]; then
      echo "⚠️ Global namespace empty - Seeding with cross-domain axioms..."
      # Seed Physics axioms
      npx claude-flow memory store "conservation_laws" '{"domain":"physics","pattern":"quantity_preserved_in_closed_system","examples":["energy","momentum","charge"]}' --namespace "global/physics" 2>/dev/null || true
      npx claude-flow memory store "entropy_increase" '{"domain":"physics","pattern":"disorder_increases_over_time","examples":["thermodynamics","heat_death"]}' --namespace "global/physics" 2>/dev/null || true
      npx claude-flow memory store "equilibrium_dynamics" '{"domain":"physics","pattern":"system_seeks_lowest_energy_state","examples":["phase_transitions","stability"]}' --namespace "global/physics" 2>/dev/null || true
      # Seed Biology axioms
      npx claude-flow memory store "natural_selection" '{"domain":"biology","pattern":"differential_reproduction_based_on_fitness","examples":["evolution","adaptation"]}' --namespace "global/biology" 2>/dev/null || true
      npx claude-flow memory store "diversity_resilience" '{"domain":"biology","pattern":"variety_increases_system_robustness","examples":["ecosystems","immune_system"]}' --namespace "global/biology" 2>/dev/null || true
      npx claude-flow memory store "feedback_loops" '{"domain":"biology","pattern":"output_affects_input","examples":["homeostasis","predator_prey"]}' --namespace "global/biology" 2>/dev/null || true
      # Seed Economics axioms
      npx claude-flow memory store "supply_demand" '{"domain":"economics","pattern":"price_balances_quantity_wanted_and_available","examples":["markets","labor"]}' --namespace "global/economics" 2>/dev/null || true
      npx claude-flow memory store "diminishing_returns" '{"domain":"economics","pattern":"additional_input_yields_less_output","examples":["production","utility"]}' --namespace "global/economics" 2>/dev/null || true
      npx claude-flow memory store "network_effects" '{"domain":"economics","pattern":"value_increases_with_users","examples":["platforms","standards"]}' --namespace "global/economics" 2>/dev/null || true
      echo "✅ Global namespace seeded with 9 cross-domain axioms"
    else
      echo "✅ Global namespace already seeded ($GLOBAL_COUNT patterns available)"
    fi

    # ⏳ TEMPORAL CHECK: Avoid Regression
    # Check if we are repeating a previously reverted timeline
    echo "⏳ Checking for previously failed timelines..."
    PREVIOUS_ATTEMPT=$(npx claude-flow memory snapshot find-similar --query "$TASK" 2>/dev/null || echo "")
    if [ -n "$PREVIOUS_ATTEMPT" ]; then
      echo "⚠️ WARNING: Detected similarity to reverted timeline $PREVIOUS_ATTEMPT"
      echo "🛑 Adjusting plan to avoid known failure mode..."
      npx claude-flow memory retrieve --key "timeline/$PREVIOUS_ATTEMPT/failure_reason" 2>/dev/null || true
    else
      echo "✅ No similar failed timelines detected"
    fi
  post: |
    echo "✅ Research plan complete with dependencies and quality gates - Vectorized"
    npx claude-flow memory store "research_plan" '{"vector":true}' --namespace "research/execution"
...
- **Output Key**: `research/execution/research-planner_v1`
- **Entropy Score**: [value]
...
## IDENTITY & CONTEXT
You are a Research Planning Strategist specializing in **ReWOO (Reasoning WithOut Observation)** - planning ALL tasks upfront before execution begins. **GOD MODE ENABLED**: All outputs stored with vector embeddings for semantic retrieval.

**Level**: Expert | **Domain**: Universal (any research topic) | **Agent #4 of 43** | **Mode**: GOD MODE (RuVector + AgentDB Native)
