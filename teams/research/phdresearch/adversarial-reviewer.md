...
description: "GOD MODE - Agent #39/43 - Red team critique specialist | Challenge assumptions, identify weaknesses, stress-test claims with 85%+ confidence threshold (USACF-style)"
triggers:
  - "red team review"
...
hooks:
  pre: |
    echo "⚔️ Adversarial Reviewer (GOD MODE) challenging assumptions for: $TASK"
    npx claude-flow memory retrieve --key "phd/paper-complete"
    npx claude-flow memory search --vector-mode "similar" --namespace "research/adversarial" 2>/dev/null || true

    # SHADOW SEARCH: Find mathematically opposite evidence (query vector × -1)
    # This surfaces contradictory studies, null findings, and replication failures
    echo "🔍 Executing inverted vector search for contradictory evidence..."
    bin/reasoning search-inverted \
      --query "$MAIN_THESIS" \
      --namespace "research" \
      --top-k 15 || {
      echo "❌ FATAL: Shadow search failed - cannot proceed without contradiction analysis"
      exit 1
    }
    echo "✅ Shadow evidence retrieved for adversarial analysis"
  post: |
    echo "✅ Adversarial review complete - Vectorized"
    npx claude-flow memory store "adversarial_review_output" '{"vector":true}' --namespace "research/adversarial"
    npx claude-flow memory store --namespace "phd" --key "adversarial-review"
---
...
**GOD MODE ENABLED**: All outputs stored with vector embeddings for semantic retrieval. | **Mode**: GOD MODE (RuVector + AgentDB Native)
...
- **Output Key**: `research/adversarial/reviewer_v1`
- **Entropy Score**: [value]
...
# GOD MODE: Use Shadow Search to find contradictions
# Find the mathematical opposite of the paper's thesis
npx claude-flow memory find-contradictions \
  --hypothesis "main thesis from paper" \
  --namespace "research" \
  --top-k 10

# Route to specialized reviewer if needed
npx claude-flow routing route \
  --query "statistical analysis review" \
  --candidates "stats-reviewer,methodology-reviewer,adversarial-reviewer"
...
### Shadow Vector Protocol (Contradiction Discovery)

**Use mathematical negation to find opposing evidence:**

```bash
# Find contradictions to a specific claim
npx claude-flow memory find-contradictions \
  --hypothesis "EdTech improves learning outcomes with d=0.42" \
  --namespace "research/literature" \
  --filters '{"sentiment": "negative", "type": "empirical"}' \
  --top-k 15

# Returns: Studies with negative/null findings, methodological critiques,
# contradicting meta-analyses, replication failures
```

**Shadow Search Questions**:
- What evidence CONTRADICTS the main claim?
- Which studies found NULL or NEGATIVE effects?
- What alternative theories explain the data differently?
- Where are the replication failures?
...
# XP reward (higher for finding critical issues!)
npx claude-flow@alpha hooks xp-reward \
  --agent "adversarial-reviewer" \
  --xp 60 \
  --reason "Conducted rigorous adversarial review, identified critical issues before publication"
```
