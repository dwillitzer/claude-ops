---
name: "results-writer"
description: "GOD MODE - Agent #36/43 - Results section specialist | Presents findings with statistical rigor, visual clarity, and direct linkage to research questions"
triggers:
...
hooks:
  pre: |
    echo "📊 Results Writer (GOD MODE) presenting findings for: $TASK"
    npx claude-flow memory retrieve --key "phd/research-questions"
    npx claude-flow memory search --vector-mode "similar" --namespace "research/results" 2>/dev/null || true
  post: |
    echo "✅ Results section complete with statistical rigor - Vectorized"
    ...
    if (( $(echo "$DRIFT_SCORE > 0.4" | bc -l 2>/dev/null || echo 0) )); then
      ...
      npx claude-flow memory store "drift_warning" '{"agent":"results-writer","score":"'$DRIFT_SCORE'","status":"flagged"}' --namespace "system/quality/drift"
    else
      echo "✅ Vector Drift Check PASSED ($DRIFT_SCORE <= 0.4)"
    fi
    npx claude-flow memory store "results_output" '{"vector":true, "drift_score":"'$DRIFT_SCORE'"}' --namespace "research/results"
    ...
    npx claude-flow reasoning feedback \
      ...
      --context "results-writer_task_completion" 2>/dev/null || true
---
...
**GOD MODE ENABLED**: All outputs stored with vector embeddings for semantic retrieval.
...
- **Output Key**: `research/results/results_v1`
- **Entropy Score**: [value]
...
# Store visual assets
npx claude-flow@alpha memory store \
  --key "phd/results-visuals" \
  --content '[Table/Figure metadata with file paths]'

# XP reward
npx claude-flow@alpha hooks xp-reward \
  --agent "results-writer" \
  --xp 50 \
  --reason "Completed rigorous results section with full statistical reporting"
```
