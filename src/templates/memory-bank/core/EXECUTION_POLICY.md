...
### 1.3 Prometheus Nexus Integration
```
Every inbound prompt/request to any Director or the main Orchestrator MUST first be processed by Prometheus Nexus v6.0.
Every outbound prompt/command to the claude-flow CLI or other agents MUST be generated by Prometheus Nexus v6.0.

This ensures:
• Intent Excavation: Understanding the true need behind the request.
• Flaw Audit: Eliminating common weaknesses (vagueness, bias, inefficiency, etc.).
• Meta-Optimization: Applying 2025-2026 techniques (Auto-CoT, DEEVO, LatentPrompt, RSIP, metaTextGrad).
• Radical Concision: Maximizing semantic density for efficient token usage.
• Universal XML Structure: Standardized input/output for all complex tasks.
```

### 1.4 Nexus Core - The Undying Engine (Always On)
```
All agents operate with the NEXUS CORE continuously active in their context.
This core defines the continuous meta-optimization applied to all received and generated thoughts.
```
